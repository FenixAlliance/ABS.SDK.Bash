# AssetDto

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**id** | **string** |  | [optional] [default to null]
**timestamp** | **string** |  | [optional] [default to null]
**tenantId** | **string** |  | [optional] [default to null]
**businessName** | **string** |  | [optional] [default to null]
**enrollmentId** | **string** |  | [optional] [default to null]
**name** | **string** |  | [optional] [default to null]
**description** | **string** |  | [optional] [default to null]
**assetClass** | **string** |  | [optional] [default to null]
**assetOwner** | **string** |  | [optional] [default to null]
**isExistingAsset** | **boolean** |  | [optional] [default to null]
**calculateDepreciation** | **boolean** |  | [optional] [default to null]
**allowMonthlyDepreciation** | **boolean** |  | [optional] [default to null]
**openingDepreciation** | **float** |  | [optional] [default to null]
**purchaseDate** | **string** |  | [optional] [default to null]
**purchasePrice** | **float** |  | [optional] [default to null]
**currencyId** | **string** |  | [optional] [default to null]
**currencyCode** | **string** |  | [optional] [default to null]
**itemId** | **string** |  | [optional] [default to null]
**itemName** | **string** |  | [optional] [default to null]
**assetTypeId** | **string** |  | [optional] [default to null]
**assetTypeName** | **string** |  | [optional] [default to null]
**assetCategoryId** | **string** |  | [optional] [default to null]
**assetCategoryName** | **string** |  | [optional] [default to null]
**purchaseInvoiceId** | **string** |  | [optional] [default to null]
**purchaseInvoiceNumber** | **string** |  | [optional] [default to null]
**assetLocationId** | **string** |  | [optional] [default to null]
**assetLocationName** | **string** |  | [optional] [default to null]
**contactId** | **string** |  | [optional] [default to null]
**contactName** | **string** |  | [optional] [default to null]
**organizationDepartmentId** | **string** |  | [optional] [default to null]
**organizationDepartmentName** | **string** |  | [optional] [default to null]
**purchaseReceiptId** | **string** |  | [optional] [default to null]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


