# ItemShippingPolicyDto

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**id** | **string** |  | [optional] [default to null]
**timestamp** | **string** |  | [optional] [default to null]
**isExpressShipmentPolicy** | **boolean** |  | [optional] [default to null]
**shippingCourierID** | **string** |  | [default to null]
**type** | **string** |  | [default to null]
**code** | **string** |  | [default to null]
**title** | **string** |  | [default to null]
**description** | **string** |  | [optional] [default to null]
**isFree** | **boolean** |  | [optional] [default to null]
**reduce** | **boolean** |  | [optional] [default to null]
**isEnabled** | **boolean** |  | [optional] [default to null]
**isDefault** | **boolean** |  | [optional] [default to null]
**allowInternational** | **boolean** |  | [optional] [default to null]
**hours** | **integer** |  | [optional] [default to null]
**days** | **integer** |  | [optional] [default to null]
**weeks** | **integer** |  | [optional] [default to null]
**months** | **integer** |  | [optional] [default to null]
**years** | **integer** |  | [optional] [default to null]
**value** | **float** |  | [optional] [default to null]
**percentage** | **float** |  | [optional] [default to null]
**currencyID** | **string** |  | [default to null]
**countryID** | **string** |  | [optional] [default to null]
**countryStateID** | **string** |  | [optional] [default to null]
**customState** | **string** |  | [optional] [default to null]
**customCity** | **string** |  | [optional] [default to null]
**cityID** | **string** |  | [optional] [default to null]
**businessID** | **string** |  | [default to null]
**businessProfileRecordID** | **string** |  | [optional] [default to null]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


