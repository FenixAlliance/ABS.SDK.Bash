...
read -r -d '' ops <<EOF
  ${CYAN}getShipmentsAsync${OFF};Retrieve a list of shipments
EOF
echo "  $ops" | column -t -s ';'
    echo ""
...
print_getShipmentsAsync_help() {
    echo ""
    echo -e "${BOLD}${WHITE}getShipmentsAsync - Retrieve a list of shipments${OFF}" | paste -sd' ' | fold -sw 80 | sed '2,$s/^/    /'
    echo -e ""
    echo -e "Retrieves a list of shipments for the specified tenant." | paste -sd' ' | fold -sw 80
    echo -e ""
    echo -e "${BOLD}${WHITE}Parameters${OFF}"
